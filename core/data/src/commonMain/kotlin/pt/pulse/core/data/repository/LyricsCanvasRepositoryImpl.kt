...
import pt.pulse.core.data.mapping.toCanvasResult
import pt.pulse.core.data.mapping.toLyrics
...
import pt.pulse.core.domain.data.model.metadata.PulseLyrics
...
import pt.pulse.core.domain.utils.toPlainLrcString
import pt.pulse.core.domain.utils.toRichSyncLrcString
import pt.pulse.core.domain.utils.toSyncedLrcString
...
import pt.pulse.service.lyrics.am.AMMotionVideo
import pt.pulse.service.lyrics.am.AMSongWithAlbum
import pt.pulse.service.lyrics.am.toImageUrl
import pt.pulse.service.lyrics.models.request.LyricsBody
import pt.pulse.service.lyrics.models.request.TranslatedLyricsBody
import pt.pulse.service.lyrics.parser.parseTtmlLyrics
...
internal class LyricsCanvasRepositoryImpl(
    ...
    // Pulse Lyrics
    private val pulseLyricsTag = "PulseLyricsRepository"

    override fun getPulseLyrics(videoId: String): Flow<Resource<Lyrics>> =
        flow {
            pulseLyrics
                .getLyrics(videoId)
                .onSuccess { lyrics ->
                    Logger.d(pulseLyricsTag, "Lyrics found: $lyrics")
                    val result = lyrics.firstOrNull()
                    if (result == null) {
                        Logger.w(pulseLyricsTag, "No lyrics found for videoId: $videoId")
                    ...
                    if (appLyrics == null) {
                        Logger.w(pulseLyricsTag, "Failed to convert lyrics for videoId: $videoId")
                    ...
                    Logger.e(pulseLyricsTag, "Get Lyrics Error: ${it.message}")
                    emit(Resource.Error<Lyrics>(it.message ?: "Failed to get lyrics"))
                }
        }.flowOn(Dispatchers.IO)

    override fun getPulseTranslatedLyrics(
        videoId: String,
        language: String,
    ): Flow<Resource<Lyrics>> =
        flow {
            pulseLyrics
                ...
                .onSuccess { lyrics ->
                    Logger.d(pulseLyricsTag, "Translated Lyrics found: ${lyrics.toLyrics()}")
                    emit(
                        Resource.Success<Lyrics>(
                            lyrics
                                .toLyrics()
                    ...
                    Logger.e(pulseLyricsTag, "Get Translated Lyrics Error: ${it.message}")
                    emit(Resource.Error<Lyrics>(it.message ?: "Failed to get translated lyrics"))
                }
        }.flowOn(Dispatchers.IO)

    override fun votePulseLyrics(
        lyricsId: String,
        upvote: Boolean,
        ...
        flow {
            pulseLyrics
                ...
                .onSuccess {
                    Logger.d(pulseLyricsTag, "Vote Lyrics Success: $it")
                    emit(Resource.Success(it.id))
                }.onFailure {
                    Logger.e(pulseLyricsTag, "Vote Lyrics Error: ${it.message}")
                    emit(Resource.Error<String>(it.message ?: "Failed to vote lyrics"))
                }
        }.flowOn(Dispatchers.IO)

    override fun votePulseTranslatedLyrics(
        ...
        flow {
            pulseLyrics
                ...
                .onSuccess {
                    Logger.d(pulseLyricsTag, "Vote Translated Lyrics Success: $it")
                    emit(Resource.Success(it.id))
                }.onFailure {
                    Logger.e(pulseLyricsTag, "Vote Translated Lyrics Error: ${it.message}")
                    emit(Resource.Error<String>(it.message ?: "Failed to vote translated lyrics"))
                }
        }.flowOn(Dispatchers.IO)

    override fun insertPulseLyrics(
        dataStoreManager: DataStoreManager,
        track: Track,
        duration: Int,
        lyrics: Lyrics,
        ...
        flow {
            ...
            pulseLyrics
                .insertLyrics(
                    ...
                    Logger.d(pulseLyricsTag, "Inserted Lyrics: $it")
                    emit(Resource.Success(it.id))
                }.onFailure {
                    Logger.e(pulseLyricsTag, "Insert Lyrics Error: ${it.message}")
                    emit(Resource.Error<String>(it.message ?: "Failed to insert lyrics"))
                }
        }.flowOn(Dispatchers.IO)

    override fun insertPulseTranslatedLyrics(
        dataStoreManager: DataStoreManager,
        track: Track,
        ...
        flow {
            ...
            pulseLyrics
                .insertTranslatedLyrics(
                    ...
                    Logger.d(pulseLyricsTag, "Inserted Translated Lyrics: $it")
                    emit(Resource.Success(it.id))
                }.onFailure {
                    Logger.e(pulseLyricsTag, "Insert Translated Lyrics Error: ${it.message}")
