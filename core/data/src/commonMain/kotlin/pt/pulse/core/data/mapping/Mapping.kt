package pt.pulse.core.data.mapping

import pt.pulse.core.data.parser.toListThumbnail
...
import pt.pulse.core.domain.data.model.searchResult.albums.AlbumsResult
import pt.pulse.core.domain.data.model.searchResult.artists.ArtistsResult
...
import pt.pulse.core.domain.data.model.searchResult.songs.SongsResult
import pt.pulse.core.domain.data.model.searchResult.songs.Thumbnail
import pt.pulse.core.domain.data.model.searchResult.videos.VideosResult
...
import pt.pulse.core.spotify.model.response.spotify.CanvasResponse
import pt.pulse.core.spotify.model.response.spotify.SpotifyLyricsResponse
import pt.pulse.service.lyrics.models.response.LyricsResponse
import pt.pulse.service.lyrics.models.response.TranslatedLyricsResponse
import pt.pulse.service.lyrics.parser.parseRichSyncLyrics
import pt.pulse.service.lyrics.parser.parseSyncedLyrics
import pt.pulse.service.lyrics.parser.parseUnsyncedLyrics
...
internal fun Track.toSongItemForDownload(): SongItem =
    SongItem(
        ...
        artists =
            this.artists?.map {
                pt.pulse.core.kotlinytmusicscraper.models.Artist(
                    id = it.id ?: "",
                    name = it.name,
                )
            } ?: emptyList(),
        album =
            pt.pulse.core.kotlinytmusicscraper.models.Album(
                id = this.album?.id ?: "",
...
internal fun pt.pulse.service.lyrics.domain.Lyrics.toLyrics(): Lyrics {
...
internal fun Lyrics.toLibraryLyrics(): pt.pulse.service.lyrics.domain.Lyrics =
    pt.pulse.service.lyrics.domain.Lyrics(
        lyrics =
            pt.pulse.service.lyrics.domain.Lyrics.LyricsX(
                lines =
                    this.lines?.map {
                        pt.pulse.service.lyrics.domain.Lyrics.LyricsX.Line(
                            endTimeMs = it.endTimeMs,
                            startTimeMs = it.startTimeMs,
                            syllables = listOf(),
                            words = it.words,
...
internal fun SearchSuggestions.toDomainSearchSuggestions(): pt.pulse.core.domain.data.model.searchResult.SearchSuggestions =
    pt.pulse.core.domain.data.model.searchResult.SearchSuggestions(
...
internal fun AccountInfo.toDomainAccountInfo(): pt.pulse.core.domain.data.model.account.AccountInfo =
    pt.pulse.core.domain.data.model.account.AccountInfo(
        name = this.name,
        email = this.email,
        pageId = this.pageId,
