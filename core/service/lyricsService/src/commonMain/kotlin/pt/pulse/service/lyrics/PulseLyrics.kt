...
import pt.pulse.service.lyrics.am.AMTokenManager
...
import pt.pulse.service.lyrics.models.request.VoteBody
...
class PulseLyrics {
    ...
    private fun createClient() =
        HttpClient(getEngine()) {
            ...
            install(HttpCache)
            install(CurlLogger) {
                logger = { Logger.d("PulseLyrics", it) }
            }
            install(HttpSend) {
                maxSendCount = 100
            }
    ...
    private fun HttpRequestBuilder.buildDefaultHeaders(
        ...
        headers {
            ...
            header(HttpHeaders.UserAgent, "PulseLyrics/1.0")
