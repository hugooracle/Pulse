...
actual class Extractor {
    ...
    actual fun mergeAudioVideoDownload(filePath: String): DownloadProgress {
        ...
        if (FileSystem.SYSTEM.exists("$filePath-Pulse.mp4".toPath())) {
            FileSystem.SYSTEM.delete("$filePath-Pulse.mp4".toPath())
        }
    ...
    actual fun saveAudioWithThumbnail(
        ...
        try {
            ...
            if (FileSystem.SYSTEM.exists("$filePath-pulse.mp3".toPath())) {
                FileSystem.SYSTEM.delete("$filePath-pulse.mp3".toPath())
            }
        } catch (e: IOException) {
            e.printStackTrace()
        }
