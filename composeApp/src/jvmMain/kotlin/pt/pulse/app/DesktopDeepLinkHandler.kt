 ...
 * - pulse://open-app?url=<encoded_url>  (redirected from website)
 * - pulse://watch?v=VIDEO_ID            (direct scheme)
 * - pulse://playlist?list=PLAYLIST_ID   (direct scheme)
 * - pulse://channel/CHANNEL_ID          (direct scheme)
 * - pulse://album?id=ALBUM_ID           (direct scheme)
 * - pulse://...            (web URL passed via args)
 */
object DesktopDeepLinkHandler {
    private const val TAG = "DesktopDeepLinkHandler"

    private val pendingUriFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "pulse_pending_deeplink.txt")
    }
     ...
     *
     * Conversion rules:
     * 1. pulse://open-app?url=<encoded_url>
     *    → Extract the `url` param and use it as intent data
     *
     * 2. pulse://watch?v=xxx, pulse://playlist?list=xxx, etc.
     *    → Convert to pulse://watch?v=xxx format
     *      so App.kt handles it uniformly via the Pulse branch
     *
     * 3. pulse://... or YouTube URLs
     *    → Pass through as-is
     */
    private fun parseToIntent(uri: String): GenericIntent {
        ...
        val actualUri = when {
            // pulse://open-app?url=<encoded_url>
            parsed.scheme == "pulse" && parsed.host == "open-app" -> {
                val urlParam = parsed.getQueryParameter("url")
                if (urlParam != null) {
                    Logger.d(TAG, "Extracted URL from open-app: $urlParam")
                    Uri.parse(urlParam)
                } else {
                    // pulse://open-app without params → just open the app, no navigation
                    Logger.d(TAG, "open-app without URL param, just opening app")
                    null
                }
            }

            // pulse://watch?v=xxx → pulse://watch?v=xxx
            // pulse://playlist?list=xxx → pulse://playlist?list=xxx
            // pulse://channel/UCxxx → pulse://channel/UCxxx
            // pulse://album?id=xxx → pulse://album?id=xxx
            parsed.scheme == "pulse" && parsed.host != null -> {
                val host = parsed.host!!
                val query = parsed.query?.let { "?$it" } ?: ""
                val pathSuffix = parsed.pathSegments.joinToString("/").let {
                    if (it.isNotEmpty()) "/$it" else ""
                }
                val convertedUrl = "pulse://$host$pathSuffix$query"
                Logger.d(TAG, "Converted pulse:// to: $convertedUrl")
            ...
            // pulse://... or YouTube URLs → pass through
            else -> parsed
        }
