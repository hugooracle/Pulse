...
/**
 * Registers the "pulse://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\pulse
 *     (Default) = "URL:Pulse Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    fun register() {
        ...
        register(SCHEME, "URL:Pulse Protocol", exePath)
        register(LASTFM_SCHEME, "URL:Pulse Last.fm Callback", exePath)
    }
    ...
    private fun resolveExePath(): String? {
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "Pulse.exe")
            if (exeFile.exists()) {
