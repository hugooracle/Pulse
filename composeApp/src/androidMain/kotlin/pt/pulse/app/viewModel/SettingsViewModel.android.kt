...
import pt.pulse.core.common.DOWNLOAD_EXOPLAYER_FOLDER
import pt.pulse.core.common.EXOPLAYER_DB_NAME
...
import pt.pulse.core.media3.di.stopService
import pt.pulse.app.extension.bytesToMB
import pt.pulse.app.extension.getSizeOfFile
import pt.pulse.app.extension.zipInputStream
...
actual suspend fun calculateDataFraction(cacheRepository: CacheRepository): SettingsStorageSectionFraction? {
    ...
    return withContext(Dispatchers.Default) {
        ...
        if (mStorageStatsManager != null) {
            ...
            val pulseSize = getSizeOfFile(application.filesDir).bytesToMB()
            val thumbSize = (application.imageLoader.diskCache?.size ?: 0L).bytesToMB()
            val otherApp = pulseSize.let { usedSpace.minus(it) - thumbSize }
            val databaseSize =
                pulseSize - playerCache.bytesToMB() - downloadCache.bytesToMB() - canvasCache.bytesToMB()
            if (totalByte ==
                freeSpace + otherApp + pulseSize + thumbSize
            ) {
