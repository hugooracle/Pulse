 ...
 *
 * Pulse Lyrics is the only provider with a vote endpoint, and the vote is cast against the
 * Pulse record itself — so the provider tag alone is NOT the condition: `pulseLyrics`
 * must actually be there. Either half qualifying is enough, because the dialog rates whichever
 * of the two came from Pulse.
 *
...
internal fun NowPlayingScreenData.LyricsData?.canVote(): Boolean {
    ...
    val votableLyrics =
        data.lyricsProvider == LyricsProvider.PULSE && data.lyrics.pulseLyrics != null
    val votableTranslation =
        data.translatedLyrics?.second == LyricsProvider.PULSE &&
            data.translatedLyrics?.first?.pulseLyrics != null
...
/**
 * Everything a Now Playing content layer reads. The shell ([pt.pulse.app.ui.screen.player.NowPlayingScreenContent])
